...
A shell command as a test backdoor, and `pytest` asserting on what the device says
from outside it. The first suite in this repo where the test process and the device
under test are two different programs.
...
`tests/drivers/gpio_button_toggle/pytest/` and talks to the device over a UART.

## What to learn here

- What `twister_harness` gives you: a `shell` fixture already connected to the
  device, whether that device is a `native_sim` process on your laptop or a board on
  a runner.
...
- What this buys you over ztest: the assertions run on a machine that can reach a
  network, a database and a file system, so an end to end test becomes possible.
...
When it goes red, that log is the answer almost every time. Knowing your way around
`twister-out/` is most of debugging a failed CI run.
